 ...
 * Compiled tests and code for Major will be stored in the `build/bin/` directory.
    * A `target` subdirectory (containing compiled subject program
      source code and compiled evosuite test files) will be generated for the purpose of measuring code coverage (via Jacoco).
 * A `libs` directory is created to hold the JAR file dependencies, which are installed to Maven's local repository.
