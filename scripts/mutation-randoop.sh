...
# Example usage:
#------------------------------------------------------------------------------
#   ./mutation-randoop.sh -c 1 commons-lang3-3.0
...
USAGE_STRING="usage: mutation-randoop.sh [-h] [-v] [-r] [-f features] [-a] [-t total_time] [-c time_per_class] [-n num_iterations] TEST-CASE-NAME
...
for i in $(seq 1 "$NUM_LOOP"); do
  for RANDOOP_FEATURE in "${RANDOOP_FEATURES[@]}"; do
    ...
    if [[ "$VERBOSE" -eq 1 ]]; then
      echo command:
      echo "$MAJOR_HOME"/bin/ant -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dmutator="mml:$MAJOR_HOME/mml/all.mml.bin" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" clean compile
    fi
    echo
    "$MAJOR_HOME"/bin/ant -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dmutator="mml:$MAJOR_HOME/mml/all.mml.bin" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" clean compile

    echo
    echo "Compiling tests..."
    if [[ "$VERBOSE" -eq 1 ]]; then
      echo command:
      echo "$MAJOR_HOME"/bin/ant -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dtest="$TEST_DIRECTORY" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" compile.tests
    fi
    echo
    "$MAJOR_HOME"/bin/ant -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dtest="$TEST_DIRECTORY" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" compile.tests

    echo
    echo "Running tests with coverage..."
    if [[ "$VERBOSE" -eq 1 ]]; then
      echo command:
      echo "$MAJOR_HOME"/bin/"$ANT" -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dmutator="mml:$MAJOR_HOME/mml/all.mml.bin" -Dtest="$TEST_DIRECTORY" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" test
    fi
    echo
    "$MAJOR_HOME"/bin/"$ANT" -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dmutator="mml:$MAJOR_HOME/mml/all.mml.bin" -Dtest="$TEST_DIRECTORY" -Dsrc="$JAVA_SRC_DIR" -lib "$LIB_ARG" test
    ...
    if [[ "$VERBOSE" -eq 1 ]]; then
      echo command:
      echo "$MAJOR_HOME"/bin/"$ANT" -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dtest="$TEST_DIRECTORY" -lib "$LIB_ARG" mutation.test
    fi
    echo
    "$MAJOR_HOME"/bin/"$ANT" -f program-config/"$1"/build-randoop.xml -Dbasedir=./ -Dtest="$TEST_DIRECTORY" -lib "$LIB_ARG" mutation.test
