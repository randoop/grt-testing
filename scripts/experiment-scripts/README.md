...
`results/[experiment]-info.csv`: the raw collected data from multiple `mutation.sh` runs.
`results/[experiment]-report.pdf`: the final rendered figure(s) and/or table(s) for the experiment.
