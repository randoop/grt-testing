...
def generate_table_3(df):
    ...
    grouped = df.groupby(['Version', 'TimeLimit']).agg({
        'InstructionCoverage': 'mean',
        'BranchCoverage': 'mean',
        'MutationScore': 'mean'
    }).reset_index()
...
def generate_fig_6(df):
    ...
    sns.boxplot(x='TimeLimit', y='InstructionCoverage', hue='Version', data=df, ax=axes[0])
...
def generate_fig_7(df):
    """
    Generate Figure 7: Box plot of branch coverage by GRT Component.
...
def generate_fig_8_9(df):
    ...
    """
    sns.set(style="whitegrid")
    grouped = df.groupby(['FileName', 'TimeLimit', 'Version'])['BranchCoverage'].mean().reset_index()
    figures = []
