...
USAGE_STRING="usage: mutation-fig7-unit.sh [-h] [-v] [-r] [-t total_time] [-c time_per_class] <test case name>"
...
if [ ! -f "results/info.csv" ]; then
    ...
    echo -e "RandoopVersion,FileName,TimeLimit,Seed,InstructionCoverage,BranchCoverage" > results/info.csv
fi
...
do
    ...
    row="$GENERATOR,$(basename "$SRC_JAR"),$TIME_LIMIT,0,$instruction_coverage%,$branch_coverage%"
    ...
    (cd results; mv jacoco.exec ../"$RESULT_DIR")
done
